...
def autoex(case, epochs=300,nbits=4,lrate=0.03,showint=100,mbs=None,vfrac=0.1,tfrac=0.1,vint=100,sm=False):
    CL = case_loader.CaseLoader()
    size = 2**nbits
    mbs = mbs if mbs else size
    
    switcher = {
        "parity": (lambda : CL.parity(2**nbits)),
        "wine": (lambda : CL.wine()),
